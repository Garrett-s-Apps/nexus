...
agent = registry.register(id="engineer", model="gpt-4")
...
- `sonnet` → `claude-sonnet-4-20250514` (latest Sonnet)
- `haiku` → `claude-haiku-4-5-20251001` (latest Haiku)

### OpenAI Provider

Support for GPT-4, GPT-3.5, o1 series:
...
result = await provider.execute(prompt, model="gpt-3.5-turbo")
result = await provider.execute(prompt, model="o1-preview")
```
...
**Cultural Diversity**: Names represent diverse backgrounds including Asian, Hispanic, African, European, and Middle Eastern cultures.
...
async def compare_providers():
    ...
    openai_result = await openai_provider.execute(task, model="gpt-4")

...
The NEXUS interactive shell is a Claude Code-like terminal experience that gives you a live REPL connected to the full 27-agent engine. Type natural language, slash commands, or questions — the shell streams agent activity back to you in real time.
...
## /99 — AI Pair Programming

Inspired by ThePrimeagen's [99 Neovim agent](https://github.com/ThePrimeagen/99), adapted for NEXUS. Use `/99` as a slash command inside the interactive shell — it launches a pair-programming session where the full NEXUS agent org executes your instructions in real time.

### Usage (inside the interactive shell)

First launch the shell, then use `/99` as a slash command:

```bash
buildwithnexus          # launch the interactive shell
```

```
nexus ❯ /99                                      # start pair-programming session
nexus ❯ /99 refactor the auth module              # one-shot instruction
nexus ❯ /99 --edit src/auth/login.ts              # AI-assisted file edit
nexus ❯ /99 --search "refresh token"              # contextual codebase search
nexus ❯ /99 --debug                               # debug current issue
```

When you send an instruction, the full NEXUS agent org executes it in real time:

```
nexus ❯ /99 add refresh token rotation to the auth service

  Chief of Staff    Routing to engineering team...
  Tech Lead         Analyzing auth module structure...
  Backend Dev       Designing token rotation schema...
  Backend Dev       Implementing refresh token logic...
  Security          Reviewing for token leakage vectors...
  Code Reviewer     Approving changes...
  ✓ Complete        2 files modified
```

### Prefix Syntax

Attach files and rules to any `/99` instruction:

- `@<filepath>` — attach a file as context: `/99 @src/auth.ts refactor to use refresh tokens`
- `#<rulefile>` — load a rules file: `/99 #security-rules add input validation`

Multiple prefixes combine freely: `/99 @src/auth.ts @src/db.ts #security-rules add rate limiting`

### Context Auto-Loading

`/99` automatically finds and loads `AGENT.md`, `SKILL.md`, and `CLAUDE.md` by walking
up from your working directory — so the agent understands your project's conventions
...
<table>
  ...
  <tbody>
    <tr>
      <td><code>/99 [instruction]</code></td>
      <td>AI pair-programming session — describe what to build or change and the full NEXUS engineering org executes it. Supports <code>@file</code> and <code>#rule</code> prefixes. Options: <code>--edit</code>, <code>--search</code>, <code>--debug</code></td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>List all 27 agents with role, model, and current state</td>
    </tr>
    <tr>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>The autonomous orchestration server running inside the VM at <code>:4200</code>. Houses all 27 agents, the ML pipeline, encrypted databases, and Docker sandboxing. Exposes a RESTful API: <code>/message</code>, <code>/events</code> (SSE), <code>/agents</code>, <code>/cost</code>, <code>/health</code>, <code>/status</code>.</td>

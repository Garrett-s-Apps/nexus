...
Inspired by ThePrimeagen's [99 Neovim agent](https://github.com/ThePrimeagen/99), adapted for NEXUS and the terminal.
NOT a Neovim plugin — `/99` runs inside Claude Code and as a `buildwithnexus` CLI command.

```bash
buildwithnexus 99                              # interactive pair-programming session
buildwithnexus 99 "refactor the auth module"   # one-shot instruction
buildwithnexus 99 --edit src/auth/login.ts     # AI-assisted file edit
buildwithnexus 99 --search "refresh token"     # contextual codebase search
buildwithnexus 99 --debug                      # debug current issue
```

When you send an instruction, the full NEXUS agent org executes it in real time:

```
99 ❯ add refresh token rotation to the auth service
...
- `@<filepath>` — attach a file as context: `@src/auth.ts refactor to use refresh tokens`
- `#<rulefile>` — load a rules file: `#security-rules add input validation`

Multiple prefixes combine freely: `@src/auth.ts @src/db.ts #security-rules add rate limiting`
